...
aiXplain_model = ModelFactory.get("6414bd3cd09663e9225130e8")  # Replace with your aiXplain model ID

# Initialize the Mistral model
mistral_model = ModelFactory.get("6551a9e7bf42e6037ab109de")  # Replace with your Mistral model ID
...
# Use aiXplain model for prediction
def run_aixplain_model(input_text):
    result = aiXplain_model.run({
        'text': input_text
    })
    
    return result['data']  # This will return the full generated text
...
def forecast():
    ...
    if request.method == 'POST':
        ...
        # Combine the inputs into a format suitable for the aiXplain model
        input_text = (f"Crop: {crop}, Soil: {soil}, Region: {region}, "
                      f"Temperature: {temperature}, Weather: {weather}")

        # Get the prediction from the aiXplain model
        prediction = run_aixplain_model(input_text)
...
def generate_report():
    ...
    # Use Mistral model for generating the report
    mistral_result = mistral_model.run({'text': text})

    if mistral_result.get('completed'):
        return jsonify({'result': mistral_result['data']})
    else:
        return jsonify({'result': 'No detailed report generated. Please check the Mistral model.'})
